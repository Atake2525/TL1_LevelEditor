...
SkinningObject3dBase* SkinningObject3dBase::instance = nullptr;

SkinningObject3dBase* SkinningObject3dBase::GetInstance() {
	if (instance == nullptr) {
		instance = new SkinningObject3dBase;
	}
	return instance;
}

void SkinningObject3dBase::Finalize() {
	delete instance;
	instance = nullptr;
}
...
void SkinningObject3dBase::CreateRootSignature() {
	...
	rootParameters[8].Descriptor.ShaderRegister = 5;                    // b4
	rootParameters[9].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE; // DescriptorTableを使う
	rootParameters[9].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX; // PixelShaderで使う
	rootParameters[9].DescriptorTable.pDescriptorRanges = &descriptorRange[0];            // Tableの中身の配列を指定
...
void SkinningObject3dBase::CreateGraphicsPipeLineState() {
	CreateRootSignature();
	// PSOを作成する
