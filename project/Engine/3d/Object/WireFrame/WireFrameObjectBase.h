...
private:
	// シングルトンパターンを適用
	static WireFrameObjectBase* instance;

	// コンストラクタ、デストラクタの隠蔽
	WireFrameObjectBase() = default;
	~WireFrameObjectBase() = default;

	// コピーコンストラクタ、コピー代入演算子の封印
	WireFrameObjectBase(WireFrameObjectBase&) = delete;
	WireFrameObjectBase& operator=(WireFrameObjectBase&) = delete;

public:
	// インスタンスの取得
	static WireFrameObjectBase* GetInstance();
